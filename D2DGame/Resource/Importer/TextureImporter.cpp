...
auto TextureImporter::Load(const std::string & path, D3D11_Texture * texture) -> const bool
{
    if (!texture)
    {
...
void TextureImporter::GenerateMipmaps(FIBITMAP * bitmap, D3D11_Texture * texture, uint width, uint height, uint channels)
{
    if (!texture)
    {
    ...
    for (uint i = 0; i < jobs.size(); i++)
        jobs[i].data = texture->GetMipLevel_ptr(i + 1);
