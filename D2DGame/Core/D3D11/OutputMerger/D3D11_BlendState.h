...
{
public:
    D3D11_BlendState(class Context* const context);
    ~D3D11_BlendState();

    auto GetResource()      const -> ID3D11BlendState* const { return state; }
    auto GetBlendFactor()   const -> const float& { return blend_factor; }
    auto GetSampleMask()    const -> const uint& { return sample_mask; }
